...
vmrule: "victoria-metrics-victoria-metrics-k8s-stack-node-exporter": {
	...
	metadata: {
		...
		name:      "victoria-metrics-victoria-metrics-k8s-stack-node-exporter"
	...
	spec: groups: [{
		...
		rules: [{
			...
			expr: "((((node_filesystem_avail_bytes{job=\"node-exporter\",fstype!=\"\",mountpoint!=\"\"} / node_filesystem_size_bytes{job=\"node-exporter\",fstype!=\"\",mountpoint!=\"\"}) * 100) < 15) and (predict_linear(node_filesystem_avail_bytes{job=\"node-exporter\",fstype!=\"\",mountpoint!=\"\"}[6h], 86400) < 0)) and (node_filesystem_readonly{job=\"node-exporter\",fstype!=\"\",mountpoint!=\"\"} == 0)"
			for:  "1h"
			...
			expr: "((((node_filesystem_avail_bytes{job=\"node-exporter\",fstype!=\"\",mountpoint!=\"\"} / node_filesystem_size_bytes{job=\"node-exporter\",fstype!=\"\",mountpoint!=\"\"}) * 100) < 10) and (predict_linear(node_filesystem_avail_bytes{job=\"node-exporter\",fstype!=\"\",mountpoint!=\"\"}[6h], 14400) < 0)) and (node_filesystem_readonly{job=\"node-exporter\",fstype!=\"\",mountpoint!=\"\"} == 0)"
			for:  "1h"
			...
			expr: "(((node_filesystem_avail_bytes{job=\"node-exporter\",fstype!=\"\",mountpoint!=\"\"} / node_filesystem_size_bytes{job=\"node-exporter\",fstype!=\"\",mountpoint!=\"\"}) * 100) < 3) and (node_filesystem_readonly{job=\"node-exporter\",fstype!=\"\",mountpoint!=\"\"} == 0)"
			for:  "30m"
			labels: severity: "critical"
		}, {
			...
			expr: "((((node_filesystem_files_free{job=\"node-exporter\",fstype!=\"\",mountpoint!=\"\"} / node_filesystem_files{job=\"node-exporter\",fstype!=\"\",mountpoint!=\"\"}) * 100) < 40) and (predict_linear(node_filesystem_files_free{job=\"node-exporter\",fstype!=\"\",mountpoint!=\"\"}[6h], 86400) < 0)) and (node_filesystem_readonly{job=\"node-exporter\",fstype!=\"\",mountpoint!=\"\"} == 0)"
			for:  "1h"
			labels: severity: "warning"
		}, {
			alert: "NodeFilesystemFilesFillingUp"
			...
			expr: "((((node_filesystem_files_free{job=\"node-exporter\",fstype!=\"\",mountpoint!=\"\"} / node_filesystem_files{job=\"node-exporter\",fstype!=\"\",mountpoint!=\"\"}) * 100) < 20) and (predict_linear(node_filesystem_files_free{job=\"node-exporter\",fstype!=\"\",mountpoint!=\"\"}[6h], 14400) < 0)) and (node_filesystem_readonly{job=\"node-exporter\",fstype!=\"\",mountpoint!=\"\"} == 0)"
			for:  "1h"
			labels: severity: "critical"
		}, {
			...
			expr: "(((node_filesystem_files_free{job=\"node-exporter\",fstype!=\"\",mountpoint!=\"\"} / node_filesystem_files{job=\"node-exporter\",fstype!=\"\",mountpoint!=\"\"}) * 100) < 5) and (node_filesystem_readonly{job=\"node-exporter\",fstype!=\"\",mountpoint!=\"\"} == 0)"
			for:  "1h"
			labels: severity: "warning"
		}, {
			alert: "NodeFilesystemAlmostOutOfFiles"
			...
			expr: "(((node_filesystem_files_free{job=\"node-exporter\",fstype!=\"\",mountpoint!=\"\"} / node_filesystem_files{job=\"node-exporter\",fstype!=\"\",mountpoint!=\"\"}) * 100) < 3) and (node_filesystem_readonly{job=\"node-exporter\",fstype!=\"\",mountpoint!=\"\"} == 0)"
			for:  "1h"
			labels: severity: "critical"
		}, {
			...
			expr: "(sum(avg(rate(node_cpu_seconds_total{job=\"node-exporter\",mode!~\"idle|iowait\"}[2m])) without(cpu)) without(mode) * 100) > 90"
			for:  "15m"
			...
			expr: "(node_load1{job=\"node-exporter\"} / count(node_cpu_seconds_total{job=\"node-exporter\",mode=\"idle\"}) without(cpu,mode)) > 2"
			for:  "15m"
			labels: severity: "warning"
		}, {
